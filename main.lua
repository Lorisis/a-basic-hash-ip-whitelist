...
local authtest = loadstring(game:HttpGet("https://pastebin.com/raw/"))()
local succ, err = pcall(function()
local succ, res = pcall(game:HttpGet("https://pastebin.com/raw/cXy4J0zg"))();
if (not succ) then
game.Players.LocalPlayer:Kick("Failed to connect to webserver.");
    else
        local body = httpService:JSONDecode(res.Body);
        local successful, signature = body.Success, body.Signature;
...
local function Equate(x)
   x = x + 62
   return (x ^ 2) + (16 * x) + 31313 + ((((x % 3) ^ 2) + ((x % 7) ^ 2)) * x)
end
local RndClass = {}
...
test = sha256(ip.."Windown"); 
for i,v in pairs(authtest) do 
    if v == test then 
         game.StarterGui:SetCore("SendNotification", {
    Title = "Window! You are now AutherRiced!";
    Text = "Enjoy!";
